...
// Every address a Chromium history holds, visits or not. The browser cleaner
// needs the adult ones by name: most that its sweeps could not reach were
// redirect steps, which Chrome keeps in the database but hides from its
// history page and from extensions (271 of 283 left on 2026-09-25).
function listUrls(source) {
  if (source.kind !== 'chromium') return [];
  const { dir, copy } = copyDatabase(source.file);
  try {
    return query(copy, 'SELECT url FROM urls', []).map((r) => String(r.url || ''));
  } finally {
    try { fs.rmSync(dir, { recursive: true, force: true }); } catch (_) {}
  }
}

module.exports = { findBrowsers, readVisits, listUrls };
